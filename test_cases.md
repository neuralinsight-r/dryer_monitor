...
| :--- | :--- | :--- | :--- | :--- |
| **TC-F01** | Happy Path: Full Cycle | `is_vibrating() == True` for > 300s, then `False` for > 120s | State transitions: `IDLE` -> `STARTING` -> `RUNNING` -> `WAITING_TO_STOP` -> `IDLE`. Buzzer triggers for 3s, SMS is dispatched, log entries verified. | **PASS** |
| **TC-F02** | Short Transient Vibration | `is_vibrating() == True` for only 45 seconds | State moves `IDLE` -> `STARTING` -> reverts to `IDLE`. No logs confirm a cycle, no buzzer or SMS fires. | **PASS** |
...
| :--- | :--- | :--- | :--- | :--- |
| **TC-N01** | Missing Environment Variables | Unset `TWILIO_AUTH_TOKEN` in terminal environment | Runtime catches `KeyError` on launch, logs error to `dryer.log`, and cleanly crashes out instead of hanging. | **PASS** |
